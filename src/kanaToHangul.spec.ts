...
describe("KanaBarum - Kana→Hangul 변환 스펙", () => {
  ...
  describe("정규화/전처리", () => {
    ...
    it("한자도 발음으로 변환하고, 괄호 속 후리가나도 변환한다", () => {
      expect(converter("誕生日(たんじょうび)")).toBe("탄죠비(탄죠비)");
      expect(converter("第3回(だいさんかい)")).toBe("다이3카이(다이상카이)");
      expect(converter("京都(きょうと)")).toBe("쿄토(쿄토)");
      expect(converter("東京駅(とうきょうえき)")).toBe("도쿄에키(도쿄에키)");
      expect(converter("日本語(にほんご)")).toBe("니홍고(니홍고)");
    });
  ...
  describe("문법/토큰 기반 휴리스틱 (조사/유성화)", () => {
    ...
    describe("경계 스트레스 (구두점/괄호/따옴표/공백)", () => {
      it("조사 치환은 괄호/따옴표/구두점 경계에서도 안정적이다", () => {
        ...
        expect(converter("東京(とうきょう)へ！")).toBe("도쿄(도쿄)에！");
      });
